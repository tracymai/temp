...
class WordSwapMaskedLM(WordSwap):
    ...
    BAE 和 CLARE 只是简单地屏蔽了我们想要替换的单词并选择由屏蔽语言模型预测的替换。
    ...
    BERT-Attack 改为在令牌级别执行替换。 对于由两个或多个子词标记组成的词，它采用搜索子词标记的前 K 个替换，并生成所有可能的前替换组合。然后，根据它们的困惑度选择前 K 个组合 使用掩码语言模型计算。
    Choose which method to use by specifying "bae" or "bert-attack" for `method` argument.
    通过为`method` 参数指定“bae”或“bert-attack”来选择要使用的方法
    ...
    Args:
        ...
        masked_language_model (Union[str|transformers.AutoModelForMaskedLM]): Either the name of pretrained masked language model from `transformers` model hub
            or the actual model. Default is `bert-base-uncased`.来自“transformers”模型中心的预训练掩码语言模型的名称或实际模型。 默认为“bert-base-uncased”。
        tokenizer (obj): The tokenizer of the corresponding model. If you passed in name of a pretrained model for `masked_language_model`,
            ...
            provide a tokenizer.对应模型的分词器。 如果您为 `masked_language_model` 传入预训练模型的名称，则可以跳过此参数，因为可以从名称中推断出正确的分词器。 但是，如果您要传递实际模型，则必须提供分词器。
        max_length (int): the max sequence length the masked language model is designed to work with. Default is 512. 掩码语言模型设计使用的最大序列长度。 默认值为 512。
        window_size (int): The number of surrounding words to include when making top word prediction.
            ...
            to the masked language model. Default is `float("inf")`, which is equivalent to using the whole text.进行顶部词预测时要包含的周围词的数量。对于每个要交换的词，我们向左取 `window_size // 2` 个单词，向右取 `window_size // 2` 个单词，并将窗口内的文本传递给 掩码语言模型。 默认为`float("inf")`，相当于使用整个文本。
        ...
        min_confidence (float): minimum confidence threshold each replacement word must pass. 每个替换词必须通过的最小置信阈值。
